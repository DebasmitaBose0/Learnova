...
describe("rateLimit module", () => {
  ...
  test("checkRateLimitFallback evicts stale keys after TTL window expires", async () => {
    vi.useFakeTimers();
    delete process.env.UPSTASH_REDIS_REST_URL;
    delete process.env.UPSTASH_REDIS_REST_TOKEN;
    process.env.MONGODB_URI = "";
    ...
    // Add user entry
    await mod.checkRateLimit("test-evict-user");
    expect(mod.fallbackRateLimitMap.has("test-evict-user")).toBe(true);

    // Fast-forward time by 61 seconds (beyond window of 60 seconds)
    vi.advanceTimersByTime(61 * 1000);

    // Check if key was evicted
    expect(mod.fallbackRateLimitMap.has("test-evict-user")).toBe(false);
    vi.useRealTimers();
  });
});
